...
function ThunderBadgeQuest:isDoable()
	if  ( self:hasMap() and not hasItem("Bike Voucher") and not ( hasItem("Bicycle") or hasItem("Yellow Bicycle") or hasItem("Blue Bicycle") or hasItem("Green Bicycle") ) ) or  getMapName() == "VermilionHouse2Bottom" then
		return true
	end
	return false
end
...
function ThunderBadgeQuest:Route5()
	if hasItem("HM05 - Flash") then 
		return moveToCell(28,0) --Cerulean City
	else
		return moveToCell(27,29) --Underground House 1
	end
end
...
function ThunderBadgeQuest:UndergroundHouse1()
	if hasItem("HM05 - Flash") then
		return moveToCell(4,10) --Route 5
	else
		return moveToCell(9,3) --Underground2
	end
end

function ThunderBadgeQuest:Underground2()
	if hasItem("HM05 - Flash") then
		return moveToCell(3,2) --Underground House 1
	else
		return moveToCell(3,43) --Underground House 2
	end
end
...
function ThunderBadgeQuest:UndergroundHouse2()
	if hasItem("HM05 - Flash") then
		...
		return moveToCell(5,10) --Route 6
	end
end
...
function ThunderBadgeQuest:DiglettsCaveEntrance2()
	return moveToCell(20,14)--Digletts Cave
end

function ThunderBadgeQuest:DiglettsCave()
	return moveToCell(27,16)--Digletts Cave Entrance 1
end

function ThunderBadgeQuest:DiglettsCaveEntrance1()
	return moveToCell(15,28)--Route 2
end
...
function ThunderBadgeQuest:PokecenterVermilion()
	...
	elseif  getTeamSize() >=3 and not hasItem("HM03 - Surf") then
		...
		self:pokecenter(8,22) --Vermilion City
	end
end
...
function ThunderBadgeQuest:VermilionCity()
	...
	if  self.registeredPokecenter ~= "Pokecenter Vermilion" or ( not game.isTeamFullyHealed() and not  hasPokemonInTeam("Ditto")  ) or ( getTeamSize() >= 3 and not hasItem("HM03 - Surf") ) then
		return moveToCell(27,21) --Pokecenter Vermilion
	elseif hasItem("Rain Badge") and not hasItem("Thunder Badge") then
		return moveToCell(26,51) --Vermilion Gym
	...
	elseif  getItemQuantity("Pokeball") <= 19 and getMoney() >= 7000  then
		return moveToMap("Vermilion Pokemart")
	elseif hasItem("Rain Badge") and not hasItem("Boulder Badge") then 
		return moveToCell(42,0) --Route 6
	...
	elseif not hasItem("Old Rod") then
		return moveToCell(30,37) --Fisherman House - Vermilion
	elseif not self:isTrainingOver() then
		return moveToCell(82,40) --Route 11
...
function ThunderBadgeQuest:Route11()
	...
	elseif not isNpcOnCell(10,13) then
		moveToCell(10,12) --Digletts Cave Entrance 2
	else
		moveToCell(0,14) --Vermilion City
	end
end
...
function ThunderBadgeQuest:FishermanHouseVermilion()
	if not hasItem("Old Rod") then
		...
		return moveToCell(5,11) --Vermilion City
	end
end
...
function ThunderBadgeQuest:VermilionGym()
	...
	if not game.isTeamFullyHealed() and not hasItem("Thunder Badge") then
 		return moveToCell(6,21) --Vermilion City
	else
		if hasItem("Thunder Badge") then
			if not hasItem("TM24 - Thunderbolt") then
